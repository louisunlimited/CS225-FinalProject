...
3. Police Training Simulator
    ...
    **Algorithm:** Depth-First-Search will be used to select the path for the criminal through a graph. The nodes will be coordinates, and the edges correspond to the length of the road between them. We will also use Nearest-Neighbor-Search to find the nearest node to the starting coordinate, if applicable.
